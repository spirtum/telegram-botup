...
def test_patterns_regexp(dispatcher):
    ...
    async def hi_message_handler(u):
        contexts.append(u)
        calls.append(hi_message_handler)

    async def drop_callback_handler(u):
        contexts.append(u)
        calls.append(drop_callback_handler)

    async def give_command_handler(u):
        contexts.append(u)
        calls.append(give_command_handler)

    async def send_inline_query_handler(u):
        contexts.append(u)
        calls.append(send_inline_query_handler)

    async def common_handler(u):
        contexts.append(u)
        calls.append(common_handler)

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(hi_dude_message_update))
    asyncio.run(dispatcher.handle_context(hi_mate_message_update))
    asyncio.run(dispatcher.handle_context(foobar_message_update))
    ...
    assert contexts[-1] is hi_dude_message_update
    asyncio.run(dispatcher.handle_context(hi_mate_message_update))
    assert calls[-1] is hi_message_handler
    assert contexts[-1] is hi_mate_message_update
    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(foobar_message_update))
    assert before_calls_counter == len(calls)
    ...
    assert contexts[-1] is foobar_message_update

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(drop_a_callback_update))
    asyncio.run(dispatcher.handle_context(drop_b_callback_update))
    asyncio.run(dispatcher.handle_context(foobar_callback_update))
    assert before_calls_counter == len(calls)
    dispatcher.register_callback_handler(re.compile('^drop'), drop_callback_handler)
    asyncio.run(dispatcher.handle_context(drop_a_callback_update))
    assert calls[-1] is drop_callback_handler
    assert contexts[-1] is drop_a_callback_update
    asyncio.run(dispatcher.handle_context(drop_b_callback_update))
    assert calls[-1] is drop_callback_handler
    assert contexts[-1] is drop_b_callback_update
    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(foobar_callback_update))
    assert before_calls_counter == len(calls)
    ...
    asyncio.run(dispatcher.handle_context(foobar_callback_update))
    assert calls[-1] is common_handler
    assert contexts[-1] is foobar_callback_update

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(give_1_command_update))
    asyncio.run(dispatcher.handle_context(give_2_command_update))
    asyncio.run(dispatcher.handle_context(foobar_command_update))
    assert before_calls_counter == len(calls)
    dispatcher.register_command_handler(re.compile('^/give'), give_command_handler)
    asyncio.run(dispatcher.handle_context(give_1_command_update))
    assert calls[-1] is give_command_handler
    assert contexts[-1] is give_1_command_update
    asyncio.run(dispatcher.handle_context(give_2_command_update))
    assert calls[-1] is give_command_handler
    assert contexts[-1] is give_2_command_update
    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(foobar_command_update))
    assert before_calls_counter == len(calls)
    ...
    asyncio.run(dispatcher.handle_context(foobar_command_update))
    assert calls[-1] is common_handler
    assert contexts[-1] is foobar_command_update
    
    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(send_a_inline_query_update))
    asyncio.run(dispatcher.handle_context(send_b_inline_query_update))
    asyncio.run(dispatcher.handle_context(foobar_inline_query_update))
    assert before_calls_counter == len(calls)
    dispatcher.register_inline_handler(re.compile('^send'), send_inline_query_handler)
    asyncio.run(dispatcher.handle_context(send_a_inline_query_update))
    assert calls[-1] is send_inline_query_handler
    assert contexts[-1] is send_a_inline_query_update
    asyncio.run(dispatcher.handle_context(send_b_inline_query_update))
    assert calls[-1] is send_inline_query_handler
    assert contexts[-1] is send_b_inline_query_update
    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(foobar_inline_query_update))
    assert before_calls_counter == len(calls)
    ...
    asyncio.run(dispatcher.handle_context(foobar_inline_query_update))
    assert calls[-1] is common_handler
    assert contexts[-1] is foobar_inline_query_update
...
def test_messages(dispatcher):
    ...
    async def b_handler(u):
        contexts.append(u)
        calls.append(b_handler)

    async def c_handler(u):
        contexts.append(u)
        calls.append(c_handler)

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(a_messsage_update))
    asyncio.run(dispatcher.handle_context(b_message_update))
    asyncio.run(dispatcher.handle_context(c_message_update))
    assert before_calls_counter == len(calls)
    ...
    assert contexts[-1] is a_messsage_update

    asyncio.run(dispatcher.handle_context(b_message_update))
    assert calls[-1] is b_handler
    assert contexts[-1] is b_message_update

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(command_update))
    assert before_calls_counter == len(calls)
    ...
    assert contexts[-1] is c_message_update
...
def test_commands(dispatcher):
    ...
    asyncio.run(dispatcher.handle_context(c_command_update))
    assert before_calls_counter == len(calls)
    ...
    asyncio.run(dispatcher.handle_context(b_command_update))
    assert calls[-1] is b_handler
    assert contexts[-1] is b_command_update

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(message_update))
    assert before_calls_counter == len(calls)
    ...
    asyncio.run(dispatcher.handle_context(c_command_update))
    assert calls[-1] is c_handler
    assert contexts[-1] is c_command_update
...
def test_callbacks(dispatcher):
    ...
    asyncio.run(dispatcher.handle_context(c_callback_update))
    assert before_calls_counter == len(calls)
    ...
    asyncio.run(dispatcher.handle_context(b_callback_update))
    assert calls[-1] is b_handler
    assert contexts[-1] is b_callback_update
    ...
    asyncio.run(dispatcher.handle_context(c_callback_update))
    assert calls[-1] is c_handler
    assert contexts[-1] is c_callback_update
...
def test_inline_query(dispatcher):
    ...
    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(a_inline_query_update))
    asyncio.run(dispatcher.handle_context(b_inline_query_update))
    asyncio.run(dispatcher.handle_context(c_inline_query_update))
    assert before_calls_counter == len(calls)
    ...
    asyncio.run(dispatcher.handle_context(a_inline_query_update))
    assert calls[-1] is a_handler
    assert contexts[-1] is a_inline_query_update

    asyncio.run(dispatcher.handle_context(b_inline_query_update))
    assert calls[-1] is b_handler
    assert contexts[-1] is b_inline_query_update
    ...
    asyncio.run(dispatcher.handle_context(c_inline_query_update))
    assert calls[-1] is c_handler
    assert contexts[-1] is c_inline_query_update
...
def test_chosen_inline_result(dispatcher):
    ...
    async def chosen_inline_result_handler(u):
        contexts.append(u)
        calls.append(chosen_inline_result_handler)

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(chosen_inline_result_update))
    assert before_calls_counter == len(calls)
    ...
    assert contexts[-1] is chosen_inline_result_update
...
def test_dice(dispatcher):
    ...
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(dice_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_dice_handler(dice_handler)
    asyncio.run(dispatcher.handle_context(dice_update))
    assert calls[-1] is dice_handler
    assert contexts[-1] is dice_update
...
def test_document(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(document_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_document_handler(document_handler)
    asyncio.run(dispatcher.handle_context(document_update))
    assert calls[-1] is document_handler
    assert contexts[-1] is document_update
...
def test_animation(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(animation_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_animation_handler(animation_handler)
    asyncio.run(dispatcher.handle_context(animation_update))
    assert calls[-1] is animation_handler
    assert contexts[-1] is animation_update
...
def test_audio(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(audio_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_audio_handler(audio_handler)
    asyncio.run(dispatcher.handle_context(audio_update))
    assert calls[-1] is audio_handler
    assert contexts[-1] is audio_update
...
def test_contact(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(contact_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_contact_handler(contact_handler)
    asyncio.run(dispatcher.handle_context(contact_update))
    assert calls[-1] is contact_handler
    assert contexts[-1] is contact_update
...
def test_new_chat_members(dispatcher):
    ...
    async def new_chat_members_handler(u):
        contexts.append(u)
...
def test_new_chat_title(dispatcher):
    ...
    async def new_chat_title_handler(u):
        contexts.append(u)
...
def test_new_chat_photo(dispatcher):
    ...
    async def new_chat_photo_handler(u):
        contexts.append(u)
...
def test_left_chat_member(dispatcher):
    ...
    async def left_chat_member_handler(u):
        contexts.append(u)
...
def test_location(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(location_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_location_handler(location_handler)
    asyncio.run(dispatcher.handle_context(location_update))
    assert calls[-1] is location_handler
    assert contexts[-1] is location_update
...
def test_poll(dispatcher):
    ...
    async def poll_handler(u):
        contexts.append(u)
        calls.append(poll_handler)

    before_calls_counter = len(calls)
    asyncio.run(dispatcher.handle_context(poll_update))
    assert before_calls_counter == len(calls)
    dispatcher.register_poll_handler(poll_handler)
    asyncio.run(dispatcher.handle_context(poll_update))
    assert calls[-1] is poll_handler
    assert contexts[-1] is poll_update
...
def test_photo(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(photo_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_photo_handler(photo_handler)
    asyncio.run(dispatcher.handle_context(photo_update))
    assert calls[-1] is photo_handler
    assert contexts[-1] is photo_update
...
def test_sticker(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(sticker_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_sticker_handler(sticker_handler)
    asyncio.run(dispatcher.handle_context(sticker_update))
    assert calls[-1] is sticker_handler
    assert contexts[-1] is sticker_update
...
def test_video(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(video_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_video_handler(video_handler)
    asyncio.run(dispatcher.handle_context(video_update))
    assert calls[-1] is video_handler
    assert contexts[-1] is video_update
...
def test_video_note(dispatcher):
    ...
    async def video_note_handler(u):
        contexts.append(u)
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(video_note_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_video_note_handler(video_note_handler)
    asyncio.run(dispatcher.handle_context(video_note_update))
    assert calls[-1] is video_note_handler
    assert contexts[-1] is video_note_update
...
def test_voice(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(voice_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_voice_handler(voice_handler)
    asyncio.run(dispatcher.handle_context(voice_update))
    assert calls[-1] is voice_handler
    assert contexts[-1] is voice_update
...
def test_venue(dispatcher):
    ...
    dispatcher.register_message_handler(re.compile('.*'), message_handler)
    asyncio.run(dispatcher.handle_context(message_update))
    asyncio.run(dispatcher.handle_context(venue_update))
    assert calls[-1] is message_handler
    assert contexts[-1] is message_update
    dispatcher.register_venue_handler(venue_handler)
    asyncio.run(dispatcher.handle_context(venue_update))
    assert calls[-1] is venue_handler
    assert contexts[-1] is venue_update
...
def test_match_sequence(dispatcher):
    ...
    async def universal_handler(u):
        contexts.append(u)
        calls.append(universal_handler)

    async def abc_handler(u):
        contexts.append(u)
    ...
    asyncio.run(dispatcher.handle_context(abc_message_update))
    assert calls[-1] is abc_handler
    assert contexts[-1] is abc_message_update
